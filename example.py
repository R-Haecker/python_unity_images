'''import dataset
data = dataset.dataset_cuboids(dataset_name = "delete", unique_data_folder = True, root_dataset_folder="~/Documents/", use_unity_build=True, debug_log=True)
dictionaries = []
data.reset_index()
data.set_config(total_cuboids=[2,12],branches=[1,6], same_material=False,CameraRadius=12, Camera_FieldofView = 100)
data.set_config(same_scale=True, scale=2, total_cuboids=2, phi=[0,360], branches=None, same_theta = True, theta=30, 
...
totalPointLights=[10,15],samePointLightColor=True,PointLightsIntensity=[15,20],PointLightsColor_r=[0.9,1],PointLightsColor_g=[0.9,1],PointLightsColor_b=[0.9,1], totalSpotLights=None, 
DirectionalLightTheta=70, DirectionalLightIntensity=3.5)
for i in range(1):
    dictionaries.append(data.parameters_to_finished_data(parameters = data.create_parameters(DirectionalLightTheta=80),save_image=True))#theta=10,scale=2,phi=45)))
